...
dialogManager = {
    ...
    checkDialogs: () => {
        ...
        $("." + me.dialogLayers[0].className).each((i, e) => {
            if (!$(e).find("." + me.dialogLayers[1].className).length) {
                //create the new dialog!
                let parent = e.parentElement;
                let prediv = e;
                for (let i = me.dialogLayers.length - 1; i >= 0; i--) {
                    let thisdiv = document.createElement("div");
                    thisdiv.classList.add(me.dialogLayers[i].className);
                ...
                if (!e.classList.contains(me.noCloseClass)){
                    let closeButton = document.createElement("div");
                    closeButton.innerText = "X";
                    closeButton.classList.add(me.closeButtonClass);
    ...
    init: function () {
        ...
        let me=dialogManager;
        me.dialogLayers.forEach((v, i) => {
            $("head").append(`<style>.` + v.className + v.styling + `</style>`);
        })
        //css for the close button
        $("head").append(`<style>.` + me.closeButtonClass + me.closeButtonStyle + `</style>`);
...
JQInit.start(dialogManager.init);
